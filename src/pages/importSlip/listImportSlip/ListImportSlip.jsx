    ...
    <>
      ...
      <div className="container_ListImportSlip">
        <div className="sub_ListImportSlip">
              ...
              >
                <option value="">-Chọn nguồn xuất</option>
                {
                  listProvider.length > 0 && listProvider.map((provider) => (
                    <option value={provider._id} key={provider._id}>{(type === "Provider" && provider?.providerName) || (type === "Agency" && provider?.agencyName)}</option>
                  ))
                }
              </select>
            </div>
          ...
          <div className="sub_2_ListImportSlip" onClick={handleSearch} style={{display: "flex",alignItems: "center", justifyContent: "center"}}>
            <span>
        ...
        <div className="sub_3_ListImportSlip">
          <p onClick={() => navigate(`/createdImportSlip/${type}`)}>+ Tạo phiếu nhập kho</p>
        </div>
              ...
              </tr>
              {
                importSlips.length > 0 && importSlips.map((importSlip, index) => (
                  <tr className="listImportSlip_tr_2" key={importSlip._id}>
                    <td className="ListImportSlip_td">{(page - 1) * limit + index + 1}</td>
                    <td className="ListImportSlip_td">{importSlip.importSlipCode}</td>
                    <td className="ListImportSlip_td">{(type === "Provider" && importSlip.providerId?.providerName) || (type === "Agency" && importSlip.agencyId?.agencyName)}</td>
                    <td className="ListImportSlip_td">{formatCurrency(importSlip.importPrice || 0)}</td>
                    <td className="ListImportSlip_td">{formatDate(importSlip.createdAt)}</td>
